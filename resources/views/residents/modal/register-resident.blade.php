<x-modal name="register-resident" maxWidth="max-w-[1188px]" focusable>
    ...
    <div class="p-8 max-h-[85vh] overflow-y-auto" 
         x-data="{ 
            ...
            familyMembers: Object.values(@js(old('members', []))).map((m, i) => ({ ...m, id: m.id || Date.now() + i })), 
            pets: Object.values(@js(old('pets', []))).map((p, i) => ({ ...p, id: p.id || Date.now() + i })), 
        ...
        <form method="POST" action="{{ route('residents.store') }}" class="mt-4">
            ...
            <div class="mb-8 border-b pb-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div><x-input-label>Nationality <span class="text-red-500">*</span></x-input-label><x-text-input name="head[nationality]" class="w-full h-10 mt-1 text-sm" placeholder="e.g. Filipino" :value="old('head.nationality')" /><x-input-error :messages="$errors->get('head.nationality')" class="mt-2" /></div>
            ...
            <div class="mb-8 border-b pb-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div x-show="ownershipStatus != ownerId && ownershipStatus != ''" class="md:col-span-4 bg-gray-50 p-4 rounded-lg border border-gray-200 mt-2">
                        ...
                        <div class="bg-gray-50 p-3 rounded-lg border border-gray-200">
                            <div class="flex gap-3 items-start">
                                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 flex-1">
                                    ...
                                    <div><x-input-label>Last Name <span class="text-red-500">*</span></x-input-label><x-text-input x-bind:name="`members[${index}][last_name]`" class="w-full mt-1 text-sm" placeholder="Dela Cruz" x-model="member.last_name" /><p x-show="getErr(`members.${index}.last_name`)" x-text="getErr(`members.${index}.last_name`)" class="text-sm text-red-600 space-y-1 mt-2"></p></div>
                                    <div><x-input-label>First Name <span class="text-red-500">*</span></x-input-label><x-text-input x-bind:name="`members[${index}][first_name]`" class="w-full mt-1 text-sm" placeholder="Juan" x-model="member.first_name" /><p x-show="getErr(`members.${index}.first_name`)" x-text="getErr(`members.${index}.first_name`)" class="text-sm text-red-600 space-y-1 mt-2"></p></div>
                                    <div><x-input-label value="Middle Name" /><x-text-input x-bind:name="`members[${index}][middle_name]`" class="w-full mt-1 text-sm" placeholder="Santos" x-model="member.middle_name" /><p x-show="getErr(`members.${index}.middle_name`)" x-text="getErr(`members.${index}.middle_name`)" class="text-sm text-red-600 space-y-1 mt-2"></p></div>
                                    <div><x-input-label value="Extension" /><x-text-input x-bind:name="`members[${index}][extension]`" class="w-full mt-1 text-sm" placeholder="Jr, Sr, III" x-model="member.extension" /><p x-show="getErr(`members.${index}.extension`)" x-text="getErr(`members.${index}.extension`)" class="text-sm text-red-600 space-y-1 mt-2"></p></div>

                                    <div><x-input-label>Place of Birth <span class="text-red-500">*</span></x-input-label><x-text-input x-bind:name="`members[${index}][birthplace]`" class="w-full mt-1 text-sm" placeholder="City, Province" x-model="member.birthplace" /><p x-show="getErr(`members.${index}.birthplace`)" x-text="getErr(`members.${index}.birthplace`)" class="text-sm text-red-600 space-y-1 mt-2"></p></div>
                                    <div><x-input-label>Date of Birth <span class="text-red-500">*</span></x-input-label><x-text-input x-bind:name="`members[${index}][birthdate]`" type="date" class="w-full mt-1 text-sm" x-model="member.birthdate" /><p x-show="getErr(`members.${index}.birthdate`)" x-text="getErr(`members.${index}.birthdate`)" class="text-sm text-red-600 space-y-1 mt-2"></p></div>
                                    ...
                                    <div>
                                        ...
                                        <x-form-select x-bind:name="`members[${index}][household_role_id]`" class="w-full h-10 mt-1 text-slate-500"
                                            ...
                                            value-column="id"
                                            placeholder="Select role"
                                            x-model="member.household_role_id"
                                    ...
                                    <div>
                                        <x-input-label>Sex <span class="text-red-500">*</span></x-input-label>
                                        <select x-bind:name="`members[${index}][sex]`" x-model="member.sex" class="w-full h-10 mt-1 text-sm text-slate-500 border border-gray-300 rounded-md shadow-sm focus:border-blue-700 focus:ring-blue-700 focus:ring-1 placeholder:text-gray-400">
                                    ...
                                    <div>
                                        <x-input-label>Civil Status <span class="text-red-500">*</span></x-input-label>
                                        <select x-bind:name="`members[${index}][civil_status]`" x-model="member.civil_status" class="w-full h-10 mt-1 text-sm text-slate-500 border border-gray-300 rounded-md shadow-sm focus:border-blue-700 focus:ring-blue-700 focus:ring-1 placeholder:text-gray-400">
                                            <option value="" disabled>Select</option>
                                    ...
                                    <div><x-input-label>Nationality <span class="text-red-500">*</span></x-input-label>
                                    <x-text-input x-bind:name="`members[${index}][nationality]`" class="w-full mt-1 text-sm" placeholder="e.g. Filipino" x-model="member.nationality" /><p x-show="getErr(`members.${index}.nationality`)" x-text="getErr(`members.${index}.nationality`)" class="text-sm text-red-600 space-y-1 mt-2"></p></div>
                                    <div class="md:col-span-1"><x-input-label value="Occupation" /><x-text-input x-bind:name="`members[${index}][occupation]`" class="w-full mt-1 text-sm" placeholder="Driver, Teacher, etc." x-model="member.occupation" /><p x-show="getErr(`members.${index}.occupation`)" x-text="getErr(`members.${index}.occupation`)" class="text-sm text-red-600 space-y-1 mt-2"></p></div>
                                    
                                    <div>
                                        <x-input-label value="Sector" />
                                        <select x-bind:name="`members[${index}][sector]`" x-model="member.sector" class="w-full h-10 mt-1 text-sm text-slate-500 border border-gray-300 rounded-md shadow-sm focus:border-blue-700 focus:ring-blue-700 focus:ring-1 placeholder:text-gray-400">
                                    ...
                                    <div>
                                        <x-input-label value="Vaccinations" />
                                        <select x-bind:name="`members[${index}][vaccination]`" x-model="member.vaccination" class="w-full h-10 mt-1 text-sm text-slate-500 border border-gray-300 rounded-md shadow-sm focus:border-blue-700 focus:ring-blue-700 focus:ring-1 placeholder:text-gray-400">
                                            <option value="None">None</option>
                                    ...
                                    <div class="md:col-span-1">
                                        <x-input-label value="Comorbidity" />
                                        <x-text-input x-bind:name="`members[${index}][comorbidity]`" class="w-full mt-1 text-sm" placeholder="Hypertension, Diabetes" x-model="member.comorbidity" />
                                    ...
                                    <div class="md:col-span-2">
                                        <x-input-label value="Maintenance" />
                                        <x-text-input x-bind:name="`members[${index}][maintenance]`" class="w-full mt-1 text-sm" placeholder="Metformin, Losartan" x-model="member.maintenance" />
                        ...
                        <div class="flex gap-3 items-start bg-gray-50 p-3 rounded-lg border border-gray-200">
                            <div class="flex-1 grid grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    <x-text-input x-bind:name="`pets[${index}][quantity]`" type="number" class="w-full h-10 mt-1 text-sm text-slate-500" x-model="pet.quantity" />
